<script lang="ts">
  ...
  */
  ...
  /*
    ...
    canonical, hreflang triad, OpenGraph, Twitter, JSON-LD) is
    derived here as a single `PageSeo` object and rendered by
    `<SeoHead>` below. Locale-specific string + URL choices live
    in `buildHomePageSeo`, not in this file.
  */
  const seo = $derived(
    buildHomePageSeo({ locale, site, bio, contact, services })
  );
